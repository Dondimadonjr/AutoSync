...
async function publicarEnInstagram(igAccountId, accessToken, mediaUrl, caption) {
  try {
    ...
    logger.info(`Creando contenedor en Instagram (${isVideo ? 'REEL' : 'IMAGE'})...`, { igAccountId });
    ...
    logger.error('Error al publicar en Instagram:', { error: errorMsg });
...
async function publicarStoryInstagram(igAccountId, accessToken, mediaUrl, isVideo = false) {
  try {
    ...
    logger.info('Creando contenedor de Story en Instagram...', { igAccountId });
    const containerRes = await axios.post(containerUrl, null, { params: containerParams });
    const creationId = containerRes.data.id;
    ...
    logger.info('Story publicada exitosamente en Instagram:', publishRes.data);
    return { postId: publishRes.data.id };
  } catch (error) {
    const errorMsg = error.response?.data?.error?.message || error.message;
    logger.error('Error al publicar Story en Instagram:', { error: errorMsg });
    throw new Error(`Instagram API Error: ${errorMsg}`);
  }
}
...
async function publicarCarruselInstagram(igAccountId, accessToken, mediaUrls, caption) {
  try {
    ...
    const errorMsg = error.response?.data?.error?.message || error.message;
    logger.error('Error al publicar carrusel en Instagram:', { error: errorMsg });
    throw new Error(`Instagram API Error: ${errorMsg}`);
  }
}
